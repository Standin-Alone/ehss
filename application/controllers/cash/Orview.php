...
class Orview extends CI_Controller 
{
	...
	public function index($id){
		...
		$pdf->MultiCell(32, 1.7, ((strpos($or[0]->paytype, 'check') !== false) ? $or[0]->drawee : ""),'','C');
		$pdf->SetXY(52,150);
	...
	function epayOR($id){
		...
		$or = $this->paymentmodel->getORbyNo($id);
		// $items = $this->paymentmodel->getORItems($id);
		$pdf = new \setasign\Fpdi\Fpdi('P','mm',array('100','150'));

		// $pageCount = $pdf->setSourceFile('assets/template/epayOR.pdf');
		$pageCount = $pdf->setSourceFile('assets/template/epayORnoFee.pdf');
		// $pageCount = $pdf->setSourceFile('assets/template/epayORNoLogo.pdf');
		$pageId = $pdf->importPage(1);
		$pdf->SetTitle('PNRI ePayment Official Receipt ');//.$or[0]->receiptno
		// $pdf->setMargins(0, 0);
		// $pdf->setMargins(7, 34);
		$pdf->addPage();
		$pdf->useImportedPage($pageId, 2, 2, 100);
		...
		// add a page 
		// $pdf->SetY(38); epay
		$pdf->SetY(38);//epayORnoFee
		$current_y = $pdf->GetY();
		$w = 4;
		$y = 52;
		// $current_x = $pdf->GetX();
		$pdf->SetFont('Arial','',5);
		$pdf->SetXY($y, $current_y);
		$pdf->MultiCell(37, $w, $or[0]->receiptno,'','C');

		$pdf->SetXY($y, 43);
		$pdf->MultiCell(37, $w, $or[0]->offdate,'','C');

		$pdf->SetXY($y, 52);
		$pdf->MultiCell(37, $w, $or[0]->EPAY_TRANSACTION_TYPE,'','C');

		$pdf->SetXY($y, 56);
		$pdf->MultiCell(37, $w, $or[0]->REF_CODE,'','C');

		$pdf->SetXY($y, 60);
		$pdf->MultiCell(37, $w, $or[0]->payor,'','C');

		$pdf->SetXY($y, 64);
		$pdf->MultiCell(37, $w, $or[0]->EMAIL,'','C');

		$pdf->SetXY($y, 72);
		$pdf->MultiCell(37, $w, $or[0]->paytype,'','C');

		$pdf->SetXY($y, 76);
		$pdf->MultiCell(37, $w, 'PhP '.number_format($or[0]->amt,2,".",","),'','C');

		$pdf->SetXY($y, 81);
		$pdf->MultiCell(37, $w, '','','C');
