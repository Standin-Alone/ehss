...
class Dtr extends CI_Controller {
	...
	function ggetDTRDates()
	{
		...
		->set_output(json_encode($this->dtrmodel->ggetDTRDates()));
	}
			...
			foreach($query_result->result() as $rows)
			{
				...
				for($i = 1; $i <= $lastday ; $i++)
				{
					...
					if($holiday["on"] > 0){
						...
						//'.$timediffam.','.$timediffpm.'  '.$cadam.','.$cadpm.'
						//'.$log['amIn'].','.$log['amOut'].','.$log['pmIn'].','.$log['pmOut'].'
						$print .= '	<td height="16" width="40" align=center>
										<font face="Arial,Helvetica,sans-serif" size="1" color="'.$trfontcol.'">
											<b>'.$otOut.'</b>
										</font>
		...
		if($query_result->num_rows() > 0)
		{
			foreach($query_result->result() as $rows)
			{
				...
				for($i = 1; $i <= $lastday ; $i++)
				{
					...
					$print .= ' <tr bgcolor="'.$trbgcol.'" style="text-align:center">
	...
	{
		$timediff = $this->timeDiff("13:00:00",$timein);
