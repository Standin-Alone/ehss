			...
			<div class="portlet light bordered">
				<div class="portlet-title">
					...
					<div class=" row">
						...
						<div class="col-md-2 col-sm-2">
							<select class="form-control select2me" id="ftrType1">
					...
					<!-- <div class=" row">
						...
						<div class="col-md-6 col-sm-6" align=left>
							<a class="btn btn-danger btn-sm" data-toggle='modal' onclick="loadOR()" href='#mOR'><i class="fa fa-file-text-o"></i> View by Receipts</a>
							<a class="btn btn-success btn-sm" data-toggle='modal' onclick="loadOR()" href='#mOR'><i class="fa fa-file-text-o"></i> Online Receipts</a>
						</div>
					</div> -->
				</div>
				<div class="portlet-body">
					<table class="table table-striped table-bordered datatables" width="100%" id="table">
						...
						<tr>
							...
							<th>Prepared By
							<th>Receipt
							<th width="190px" style="min-width:190px;max-width:190px;">Action
						</tr>
						</thead>
						<tbody>
						</tbody>
...
<script language="JavaScript" type="text/javascript">
	...
	$('#soaFrom, #soaTo, #ftrType1').live('change', function(){
	...
	$(document).on('keyup',function(e){
		...
		if(!$(e.target).is(':input') || e.target.classList.contains('inpK'))
		{
			...
			if ((e.keyCode || e.which) == 79){
				try{
					$('#ptonline').attr("checked", !$('#ptonline').attr("checked"));
				} catch(err)
				{console.log(err)}
			}
			$.uniform.update();
	...
	function loadSOA()
	{
	    ...
	    var fr = $('#soaFrom').val();
	    var to = $('#soaTo').val();
	    var ftrType = $('#ftrType1').val();
	    $('#table').dataTable({
	        "serverSide": true,
	        "sAjaxSource": "<?php echo site_url('cash/payment/read/soaonline/1/"+ fr +"/"+ to +"/"+ ftrType +"'); ?>",
	        "deferLoading": 10,
	        "bStateSave": true,
	...
	function loadOR()
	{
	    ...
	    var ftrType = $('#ftrType').val();
	    $('#tableOR').dataTable({
	        "serverSide": true,
	        "sAjaxSource": "<?php echo site_url('cash/payment/read/receipt/"+ fund +"/"+ fr +"/"+ to +"/"+ ftrType +"'); ?>",
	        "deferLoading": 57,
		  ...
		  success: function(d){
			...
			if(payid){
				if(payid==1){
					...
					$.ajax({
						type:"GET", async: true, dataType: 'json', cache: false,
						url: 'https://remittance.itdi.ph/api/collections',
						"crossDomain": true,
						"headers": {
							"Authorization": "Bearer BT3PAfFSHQq-BXQl9EBg45E4l-9sL_Lg"
						},
						type: 'GET',
						beforeSend: function (xhr) {
							xhr.setRequestHeader('Authorization', 'Bearer BT3PAfFSHQq-BXQl9EBg45E4l-9sL_Lg');
						},
						success: function (data) {
							eprc = "PNRI-"+d.REF_CODE;//"PNRI-NSD-RPSS-21-3482"
							var epay = $.grep(data, function(obj){return obj.reference_number == eprc;});
							if(epay){
								if(epay[0]['status'] == "100")
									$(".epaystat").html('<span class="label label-sm label-success "> '+getMess(epay[0]['status']) +'</span>'+ ' '+ epay[0]['transaction_type']);
								else
									$(".epaystat").html('<span class="label label-sm label-danger "> '+getMess(epay[0]['status']) +'</span>'+ ' '+ epay[0]['transaction_type']);
								$("#payid").val("");
								$("#offdate").val(epay[0]['created_date']);
								$("#offtime").val(epay[0]['timestamp'].substr(11,5));
								$("#rem").val("Paid: "+epay[0]['total_amount']);
							}
							$.uniform.update();
							
						},
						error: function () { toastr.error('Please contact ITDI Administrators.', 'Ooooophs! We encountered a problem') },
					});
				}else{
					$.ajax({
					...
					success: function(d){
						...
						$('#ptonline').prop('checked', $("#type").val().includes('online'))
						$.uniform.update();
						chkChanged()
	...
	function printOR(or, ortmp)
	{
		var URL = ortmp ? "<?php echo base_url();?>cash/orview/epayOR/"+or : "<?php echo base_url();?>cash/payment/officialreceipt/"+or;
	    	...
	    	if($("#form_sample_2").valid())
	    	{
	            ...
	            ({  type:"POST",
	                ...
	                success: function(data)
	                {
						...
						if(msg == "created" && $("#type").val()=='online')
						$.ajax
						({ type:"POST",
							async: true,
							url:"<?php echo site_url('cash/payment/notifySection'); ?>",
							data: {soaid: $("#soaid").val()},
							dataType: 'json',
							cache: false,
							success: function(data){},
							error: function(data){}
	...
	function openMonthly()
	{
		...
		var to = $('#orTo').val();
		var ftrType = $('#ftrType').val();
		var URL = "<?php echo base_url();?>cash/payment/monthly/"+fund+"/"+fr+"/"+to+"/"+ftrType+"/";
	...
	function exportCashBook()
	{
		...
		var to = $('#orTo').val();
		var ftrType = $('#ftrType').val();
		var URL = "<?php echo base_url();?>cash/payment/exportCashBook/"+fund+"/"+fr+"/"+to+"/"+ftrType+"/";
	...
	function openEpay()
	{
		...
		var to = $('#orTo').val();
		var ftrType = $('#ftrType').val();
		var URL = "<?php echo base_url();?>cash/payment/exportEPay/"+fund+"/"+fr+"/"+to+"/"+ftrType+"/";
		var win = window.open(URL, 'Print EPay Report','');
	}
	...
	function openDaily1()
	{
		...
		var to = $('#orTo').val();
		var ftrType = $('#ftrType').val();
		var URL = "<?php echo base_url();?>cash/payment/daily1/"+fund+"/"+fr+"/"+ftrType+"/";
								...
								<div class="portlet-body">
									<div class="static-info">
										...
										<div class="row">
											...
											<div class="col-md-7 value epaystat"></div>
										</div>
									</div>
								</div>
							</div>
							...
							<div class="form-group static-info">
								...
								* </span>
								...
								<div class="col-md-2">
									...
									<label><input type="checkbox" name="paytype" id="ptonline" value="online" data-title="Online" class="inpK"/><u>O</u>nline </label>
								</div>
							</div>
						...
						<div class="form-body">
							<div class="form-group">
								...
								<div class="col-md-2">
									...
									<span class="help-block">fund type </span>
								</div>
								<!-- <label class="control-label col-md-2">Payment</label> -->
								<div class="col-md-2">
									<select class="form-control select2me" id="ftrType" onchange="loadOR()">
										<option value=0>all</option>
										<option value=2>onsite</option>
										<option value=1>online</option>
									</select>
								...
								<div class="col-md-2">
									...
									<span class="help-block">Input date range </span>
								</div>
							</div>
						</div>
					</form>
					...
					<div class="portlet">
						<div class="portlet-title">
							...
							<div class="actions clsPrintAct">
								...
								<span class="label label-sm label-success" onclick="openEpay()"><i class="fa fa-print"></i> Online </span>&nbsp; 
							</div>
						</div>
...
function ePayAdd() {
	$.ajax ({  type:"POST", async: true, dataType: 'json', cache: false,
				data:{
						merchant_code:'2021030672', 
						merchant_reference_number:'PNRI-NSD-RPSS-21-0002',
						particulars:'Transaction_type=TechnicalServices;Reference Number=PNRI-NSD-RPSS-21-0002;Payor Name=Juan Dela Cruz;Email Address=juandelacruz@email.com;',
						amount:'500.00',
						hash:"<?php echo md5('2021030672'.'Juan Dela Cruz'.'PNRI-NSD-RPSS-21-0002'.'500.00'.'CAXLT86Mvt');?>",
						status:'101'
						},
				url:"https://mctest.itdi.ph/connection/testing",
...
function ePayVerify(mrn) {
	$.ajax ({  type:"POST", async: true, dataType: 'json', cache: false,
            data:{
                    merchant_code:'2021030672', 
                    merchant_reference_number:mrn,
                    hash:"<?php echo md5('2021030672'.date('Y-m-d').'dost_pnri');?>"
                    },
             url:"https://mctest.itdi.ph/connection/verify",
      complete: function(data){
		getMessage(data)
      },
      error: function(data){console.log("We encountered a problem!")}
...
function ePayRemittance() {
	$.ajax({
		...
		success: function () { },
		error: function () {  toastr.error('Please contact ITDI Administrator.', 'Ooooophs! We encountered a problem')  },
	});
}
...
function getMessage(data){
	...
	switch(data.message){
		...
		case "02":
			...
			break;
		case "03":
			console.log("Invvalid Merchant Code")
			return "Success";
			break;
		case "04":
			console.log("Invalid Merchant Reference Number")
			return "Success";
			break;
		case "05":
			console.log("Invalid Particulars")
			return "Success";
			break;
		case "06":
		...
		case "101":
			...
			break;
		default:
		console.log('Please reload the page and try again.', 'Ooooophs! We encountered a problem') 
	}
}
...
jQuery(function($) {
	...
	// onlineVerify()
	// testMyApi()
    console.log("<?php echo md5('2021030672'.'Juan Dela Cruz'.'PNRI-TEST-RN'.'500.00'.'CAXLT86Mvt');?>")
})
...
function testMyApi() {
	$.ajax ({  type:"POST", async: true, dataType: 'json', cache: false,
            data:{
					epp:'12345678', 
                    merchant_reference_number:"PNRI-NSD-RPSS-21-0001",
                    hash:"<?php echo md5('2021030672'.date('m-d-Y').'CAXLT86Mvt');?>"
                },
             url:"https://admin.services.pnri.dost.gov.ph/infosys/cash/api",
      complete: function(data){
      },
      error: function(data){console.log("We encountered a problem!")}
	});
...
function printSOC(id)
{
	...
	var URL = "https://admin.services.pnri.dost.gov.ph/soa/exports/print_statement_cash/";
	// var URL = "http://10.127.44.9/soa/exports/print_statement_cash/";
	$.redirect(URL, d, "POST", "_blank");
